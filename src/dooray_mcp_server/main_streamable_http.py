"""Main entry point for Dooray MCP server (Streamable HTTP mode)."""
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Dooray MCP Server with Streamable HTTP Transport (MCP Standard)"
    )
...
async def main():
    ...
    logger.info("🚀 Starting Dooray Streamable HTTP MCP Server (MCP Standard)",
    ...
    try:
        ...
        await server.start_streamable_http_server(
            host=args.host,
            port=args.port,
            path=args.path
        )
        ...
        logger.info("🌐 Streamable HTTP Server is running. Press Ctrl+C to stop.")
        while True:
...
def cli_main():
    ...
    try:
        ...
        print("\n🛑 Streamable HTTP server stopped", file=sys.stderr)
    except Exception as e:
